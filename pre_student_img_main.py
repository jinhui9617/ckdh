...
def main():
    ...
    bone = ImgBackbone(args.num_label)
    bone.to(device)
    ...
    base_params_id_v = list(map(id, bone.alexnet.parameters()))
    base_params_id = base_params_id_v
    tune_params = filter(lambda p: id(p) not in base_params_id, bone.parameters())
    base_params = filter(lambda p: id(p) in base_params_id, bone.parameters())

    optimizer = optim.SGD([{'params': base_params}, {'params': tune_params, 'lr': 0.1}],
